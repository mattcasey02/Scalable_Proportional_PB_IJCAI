"""
Run Exact Equal Shares with completion (cost/uniform utilities, non-exhaustive, heuristic).

Uses ADD-OPT-SKIP to find minimum budget increases until budget is exhausted.
"""
...
def exact_method_of_equal_shares_with_completion_cost_heuristic(pabulib_file: str, budget: int = 0):
    """
    Run EES with budget completion using ADD-OPT-SKIP heuristic (non-exhaustive, cost utilities).
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python run_cost_equal_shares_non_exhaustive_heuristic.py <file_path>")
        sys.exit(1)
    ...
    results_dir = setup_results_dir("exact_equal_shares/cost/non_exhaustive_heuristic")
