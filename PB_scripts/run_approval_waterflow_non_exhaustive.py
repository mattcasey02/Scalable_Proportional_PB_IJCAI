"""
Run Method of Equal Shares (waterflow) with budget exhaustion (approval utilities).
...
def run_mes_with_exhaustion(pabulib_file: str, budget: int = 0) -> pd.DataFrame:
    """
    Run MES with budget exhaustion (non-exhaustive, approval utilities).
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python run_approval_waterflow_non_exhaustive.py <file_path>")
        sys.exit(1)
    ...
    results_dir = setup_results_dir("waterflow_equal_shares/approval/non_exhaustive")
