"""
Run Exact Equal Shares with completion (cost/uniform utilities, exhaustive).

Uses ADD-OPT and continues until all projects are selected.
"""
...
from core.add_opt import add_opt_cost
from core.cli import setup_results_dir, save_results


def exact_method_of_equal_shares_with_completion_cost_exhaustive(pabulib_file: str, budget: int = 0):
    """
    Run EES with budget completion using ADD-OPT (exhaustive, cost utilities).
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python run_cost_equal_shares_exhaustive.py <file_path>")
        sys.exit(1)
    ...
    results_dir = setup_results_dir("exact_equal_shares/cost/exhaustive")
