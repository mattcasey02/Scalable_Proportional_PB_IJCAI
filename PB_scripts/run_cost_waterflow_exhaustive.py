"""
Run Method of Equal Shares (waterflow) with exhaustive budget increases (cost utilities).
...
def run_mes_exhaustive(pabulib_file: str, budget: int = 0) -> pd.DataFrame:
    """
    Run MES with exhaustive budget increases (cost utilities).
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python run_cost_waterflow_exhaustive.py <file_path>")
        sys.exit(1)
    ...
    results_dir = setup_results_dir("waterflow_equal_shares/cost/exhaustive")
