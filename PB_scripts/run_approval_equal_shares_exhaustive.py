"""
Run Exact Equal Shares with completion (approval utilities, exhaustive).

Uses ADD-OPT and continues until all projects are selected, tracking monotonicity violations.
"""
...
def exact_method_of_equal_shares_with_completion_approval_exhaustive(pabulib_file: str, budget: int = 0):
    """
    Run EES with budget completion using ADD-OPT (exhaustive).
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python run_approval_equal_shares_exhaustive.py <file_path>")
        sys.exit(1)
    ...
    results_dir = setup_results_dir("exact_equal_shares/approval/exhaustive")
    ...
    try:
        ...
        output_df = exact_method_of_equal_shares_with_completion_approval_exhaustive(str(input_path))
