"""
Run Exact Equal Shares without completion (cost utilities).
...
from core.ees import exact_method_of_equal_shares_cost, create_ees_results_df
...
def exact_method_of_equal_shares_with_completion_cost(pabulib_file: str, budget: int = 0):
    ...
    # Initial EES run
    selected_projects, payments, shares, total_cost = exact_method_of_equal_shares_cost(
        instance, profile
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python run_cost_equal_shares_no_completion.py <file_path>")
        sys.exit(1)
    ...
    results_dir = setup_results_dir("exact_equal_shares/cost/no_completion")
